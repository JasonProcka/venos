...
var createHubObject = function(id, name, description, url, ownerUid, destructionTimeInHours, isPublic){
    return {
        ['id']: id,
        [KEY_NAME]: name,
...
var addFileToHub = function (userId, hubid, file) {
    ...
    //   }
    // });
        // var promise1 = Firebase.database().ref(KEY_FILES + NODE_SEP + KEY_USER + NODE_SEP + userId).push().set(file);
        // var promise3 = Firebase.database().ref(KEY_USER + NODE_SEP + userId + NODE_SEP + KEY_FILES).push().set(file);
